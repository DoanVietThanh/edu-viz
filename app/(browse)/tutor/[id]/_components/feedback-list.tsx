...
const FeedbackList = ({ selectedPackage }: FeedbackListProps) => {
  ...
  useEffect(() => {
    const fetchFeedbacks = async () => {
      const data = await getPackageFeedbacks(selectedPackage.id, selectedPackage.tutorId)
      setFeedbacks(data)
    }
    fetchFeedbacks()
  }, [selectedPackage])
      ...
      {feedbacks?.map((feedbackItem: any, index: number) =>
        feedbackItem.feedback ? (
          <div key={index} className={`flex flex-col gap-2 rounded-2xl border p-4 shadow-lg hover:bg-slate-100`}>
            <div className='flex items-center justify-between gap-4 px-4'>
              <div className='flex items-center gap-4'>
                ...
                <div className='flex flex-col'>
                  <div className='flex items-center gap-4'>
                    <div className='font-serif text-xl font-semibold'>{feedbackItem.student.fullName}</div>
                    <div className='font-semibold'>{new Date().toDateString()}</div>
                  </div>
                  <div className='italic'>
                    Ratings {Array.from({ length: feedbackItem.feedback.value || 5 }).map(() => '⭐️')}
                  </div>
                </div>
              </div>
              <div className='flex items-center gap-4'>
            ...
            <div className='flex gap-4 p-4 font-medium'>{feedbackItem?.feedback?.content}</div>
          </div>
        ) : null
      )}
    </div>
