...
function CompleteMessage({ message, otherUser }: Props) {
  ...
  return (
    <div className='flex min-h-full w-full flex-col gap-y-1 rounded-lg border-2 border-primary bg-muted p-4'>
      <div className='font-bold'>{isTutor ? otherUser.fullName : 'You'} has completed the reservation</div>
      ...
      <div className='flex items-center gap-1'>
        {isTutor ? (
          ...
          <div>Hope you had a great time studying.</div>
        )}
      </div>
    </div>
  )
